...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   for(curr = g->connections[start]; curr != NULL; curr = curr->next){
      ...
      if (curr->v == end) type[k++] = curr->type;
   }

   //checking if there is boat connection
   VList curr2 = malloc(sizeof(struct vNode));
   assert(curr2 != NULL);
   VList curr3 = malloc(sizeof(struct vNode));
   assert(curr3 != NULL);

   for(curr2 = g->connections[start]; curr2 != NULL; curr2 = curr2->next){

      if (curr2->type == BOAT)   break;
   }

   for(curr3 = g->connections[end]; curr3 != NULL; curr3 = curr3->next){

      if (curr3->type == BOAT)   break;
   }

   if (curr2 != NULL && curr3 != NULL && curr3->v == curr2->v) type[k++] = BOAT;


   return k;  // to keep the compiler happy
}
