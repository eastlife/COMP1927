...
{
	struct IntListNode *n, *pf, *pb, *pv; //two steps a loop, pf is the front one and pb is the back one, pv is the front one in previous loop
    ...
    pb = L->first;
    pv = NULL;
    for (; pf != NULL && pf->next != NULL; pf=pf->next->next){
        if (v <= pf->data){
            if (v <= pb->data){         //v is smaller than pb
                pv->next = n;
                n->next = pb;
            }
                else {                  //v is between pb and pf
                    pb->next = n;
                    n->next = pf;
                }
            return;
	...
	if (pf == NULL){
        if (v <= pb->data){         //v is the second largest
            pv->next = n;
            n->next = pb;
        } else {                  //v is the largest
            pb->next = n;
            L->last = n;
        }
    ...
    if (pf->next == NULL){
   	if (v <= pb->data) {                //v is the third largest
			pv->next = n;
			n->next = pb;
		} else if(v <= pf->data){       //v is the second largest
		    pb->next = n;
		    n->next = L->last;
		}
		else{                           //v is the largest
