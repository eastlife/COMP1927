...
// return 1 if reach end of list during move malloc(sizeof(DLListNode));
...
void DLListBefore(DLList L, char *it)
{
    ...
    //current is in the middle or last
    else {
        L->curr->prev->next = p;
        p->next = L->curr;
        p->prev = L->curr->prev;
...
void DLListDelete(DLList L)
{
	...
	// empty DDList or only one item
	if (L->curr == NULL || (L->curr->prev == NULL && L->curr->next == NULL)){
